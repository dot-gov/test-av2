...
class MailSender(object):
    ...
    def generate_text(self):
        ...
        mail_message += '<br><a id="retests"><div class="boldback">Retests to be run</div></a><br>'
        mail_message += self.retestlist

        mail_message += '<br><a id="rite_fails"><div class="boldback" style="color:darkred">RITE NEW FAILS - these are the tests in which the test system (rite) failed</div></a><br>'
        ...
        mail_message += '<br><a id="rite_known_fails"><div class="boldback" style="color:green">RITE KNOWN FAILS - these are the tests in which the test system (rite) failed but we previously aknowledged that</div></a><br>'
        for vm in sorted(self.rite_known_fails):
            ...
            mail_message += "</p></details><hr>"
        mail_message += '<br><a id="errors"><div class="boldback" style="color:red">NEW ERRORS - these are the tests in which the test FAILED</div></a><br>'
        for vm in sorted(self.errors):
            ...
            mail_message += "\n</p></details><hr>"
        mail_message += '<br><a id="known_errors"><div class="boldback" style="color:green">KNOWN ERRORS - these are the tests in which the test failed, but we previously aknowledged that</div></a><br>'
        for vm in sorted(self.known_errors):
            ...
            mail_message += "</p></details><hr>"
        mail_message += '<br><a id="ok"><div class="boldback" style="color:green">OK - Passed tests</div></a><br>'
        for vm in sorted(self.ok):
            ...
            mail_message += "</p></details><hr>"
        mail_message += '<br><a id="errorsdetails"><div class="boldback" style="color:red">ERROR DETAILS - log of every operation and python code to acknowledge the error</div></a><br>'
        for vm in sorted(self.errors):
            mail_message += "<details close><summary>Analyzed VM: %s</summary><p><br>" % vm
            for test in self.errors[vm]:
                mail_message += '<p class="tab">Test: %s</p><br>' % test
                mail_message += '<p class="doubletab">%s</p><br>' % cgi.escape(self.errors[vm][test][0])
                # mail_message += "<hr>"
    ...
    def get_html_header(self):
        ...
        <style>
            ...
            .percentbar { background:#FFFFFF; border:1px solid #000000; height:10px; margin-left: auto;
                          margin-right: auto; box-shadow: 3px 3px 2px #888888; }
            .percentbar div { background: #e5ff00; height: 10px; }
        </style>
        </head>
        <body>
        '''
    ...
    def generate_stats(self):
        ...
        if self.total_tests_num > 0:
            self.sanity = round((((self.ok_num+self.known_errors_num+self.rite_known_fails_num)*100.0)/(self.total_tests_num*100.0))*100, 2)
        else:
            self.sanity = "Unknown"
        ...
        if not self.sanity == "Unknown":
            ...
            stat_text += "Unknown Sanity (0 test runs reported from Rite)"
